...
public interface MovieRepository extends JpaRepository<Movie, Long>, JpaSpecificationExecutor<Movie> {
    ...
    // List<Movie> findByCategoryAndKeyword(@Param("categoryName") String
    // categoryName,
    // @Param("keyword") String keyword);

    // @Query("SELECT m FROM Movie m JOIN m.categories c " +
    // "WHERE c.name = :categoryName OR :keyword MEMBER OF m.keywords")
    // List<Movie> findByCategoryOrKeyword(@Param("categoryName") String
    // categoryName,
    // @Param("keyword") String keyword);

    // @Query("SELECT m FROM Movie m JOIN m.categories c " +
    // "WHERE c.name = :categoryName OR :keyword MEMBER OF m.keywords OR
    // LOWER(m.title) LIKE LOWER(CONCAT('%', :title, '%'))")
    // List<Movie> findByCategoryOrKeywordOrTitle(@Param("categoryName") String
    // categoryName,
    ...
    // @Query("SELECT m FROM Movie m JOIN m.categories c WHERE c.name =
    // :categoryName")
