...
public class AuthController {
  ...
  public ResponseEntity<?> login(@RequestBody LoginRequest authRequest, HttpServletResponse response) {
    try {
      ...
      String accessToken = jwtUtil.generateToken(userDetails, 1000 * 60 * 60); // sat vremena access token
      String refreshToken = jwtUtil.generateTokenFromUsername(userDetails.getUsername(), 1000 * 60 * 60 * 6); // 6 sati
                                                                                                              // refresh
                                                                                                              // token
